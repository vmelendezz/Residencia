...
if($_SERVER['REQUEST_METHOD']== 'POST')
{
    ...
    else
    {
        ...
        try 
        {
            ...
            $statement = $conexion->prepare("Select * from user where user = :usuario limit 1");
            //se ejecuta
    ...
    if($errores == '')
    {
        ...
        $statement = $conexion->prepare('INSERT INTO user (id,user,contrasena,tipo) VALUES(null,:usuario,:pass,:tipo)');
        //se ejecuta la consulta pasando los parametros correspondientes
        $statement->execute(array(':usuario' => $usuario,':pass' => $password, ':tipo' => "admin"));                            
